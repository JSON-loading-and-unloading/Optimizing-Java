...
자바 가비지 수집의 요체 : 시스템에 있는 모든 객체의 수명을 정확히 몰라도 런타임이 대신 객체를 추적하며 쓸모없는 객체를 알아서 제거하는 것.</br>
가비지 수집 구현체 기본 원칙!
 ...
 - 살아 있는 객체는 절대로 수집해선 안 된다.
   ...
   GC 전체 알고리즘
    ...
    4. 할당 리스트를 순회 하면서 마크 비트가 세팅되지 않는 객체를 찾는다.
        - 힙에서 메모리를 회수해 프리 리스트에 되돌린다.(메모리 회수)  =>이 단계에서는 더 이상 필요 없는 객체들의 메모리를 다시 사용 가능한 상태로 만듭니다. 여기서 "프리 리스트"란, 메모리 관리를 위해 사용 가능한 메모리 블록들을 추적하는 자료 구조를 가리킵니다.
        - 할당 리스트에서 객체를 삭제한다.
 ...
 <h4>가비지 컬렉션의 주요 단계</h4>

1.마크 단계 (Mark Phase): GC 루트(root)에서부터 시작하여, 어떤 객체가 여전히 참조되고 있는지 마크합니다. 이는 더 이상 접근할 수 없는 객체들을 식별하기 위한 단계입니다.</br>

2.스위핑 단계 (Sweep Phase): 할당된 메모리 공간 전체를 검사하면서 마크되지 않은 객체들을 찾아내고, 이들의 메모리를 회수합니다. 이때 회수된 메모리는 다시 재사용 가능한 상태로 만듭니다.</br>

3.프리 리스트 구축 (Free List Construction): 스위핑 단계에서 회수된 메모리 블록들은 프리 리스트에 추가됩니다. 이는 나중에 객체가 새로 할당되어야 할 때, 이 프리 리스트에서 빠르게 사용 가능한 메모리 블록을 찾아서 할당하는 데 사용됩니다. 이를 통해 메모리 할당의 오버헤드를 줄이고, 힙 내부의 메모리 단편화 문제를 완화할 수 있습니다.</br>

4.할당 리스트 업데이트 (Allocation List Update): 스위핑 및 프리 리스트 구축 단계 후, 더 이상 필요 없는 객체들은 할당 리스트에서 삭제됩니다. 이 객체들이 차지하던 메모리 공간은 프리 리스트에 되돌려집니다.</br>
...
=> 객체가 생성된 후 잠시 존재하고 그 상태를 보관하는데 사용한 메모리를 다시 회수한다는 발상이 핵심</br>
    할당률 : 일정기간 새로 생성된 객체가 사용한 메모리량이다.</br>
      ...
      <h3>약한 세대별 가설</h3>
       ...
       - 객체마다 '세대 카운트'를 센다</br>
       - 큰 객체를 제외한 나머지 객체는 에덴 공간에 생성한다. 여기서 살아남은 객체는 다른 곳을 옮긴다.</br>
       - 장수했다고 할 정도로 충분히 오래 살아남은 객체들은 별도의 메모리 영역에 보관한다.</br>
...
<h3>반구형 수집</h3>   
 ...
 기본적인 특성</br>
  ...
  graphs에 있는 문자? 들은 어떻게 읽는걸까요?,,ㅜㅜ</br>
   ...
   자바 8이전까지 JVM 디폴트 가비지 수집기는 병렬 수집기였다. 모두 풀 stw(애플리케이션 스레드를 모두 중단시킨 다음 실행)를 일으킨다.</br></br>

   ParallelGC : 가장 단순한 영 세대용 병렬 수집기이다.</br>
   ParNewGC : cms 수집기와 함께 사용할 수 있게 parallelGC를 조금 변형한 것이다.</br>
   ParallelOldGC : 올드 세대용 병렬 수집기이다.</br>

  <h3>영 세대 병렬 수집</h3>  

  문제 : 스레드가 에덴에 객체를 할당하려는데 자신이 할당받은 TLAB 공간이 부족하고 JVM은 새 TLAB을 할당할 수 없을 때</br>
  발생 : 영 세대 수집 </br>
  해결 : </br>
         1. jvm은 어쩔 수 없이 전체 애플리케이션 스레드를 중단시킴.</br>
         2. 핫스팟은 영 세대를 뒤져 가비지 아닌 객체를 골라냄.(GC루트를 병렬 마킹 스캔 작업의 출발점으로 삼는다.)</br>
         3. ParallelGC는 살아남은 객체를 현재 비어 있는 서바이버 공간으로 모두 방출한 후 세대 카운트를 늘린다.</br>
         4. 에덴과 이제 막 객체들을 방출시킨 서바이버 공간을 재사용 가능한 빈 공간으로 표시 후 애플리케이션 스레드를 재시작해 TLAB를 애플리케이션 스레드에 배포하는 프로세스를 함</br>
🎊살아있는 개체만 건드려 약한 세대별 가설의 이점을 활용

  <h3>올드 세대 병렬 수집</h3>

  ParallelGC와 비슷하지만 차이점이 있다!!</br>
  parallelGC는 객체를 방출하는 반구형 수집기이지만, ParallelOldGC는 하나의 연속된 메모리 공간에서 압착하는 수집기이다.</br></br>

  올드 세대에 더 이상 방출할 공간이 없으면 병렬 수집기는 올드 세대 내부에서 객체들을 재배치해서 늙은 객체가 죽고 빠져 버려진 공간을 회수한다.</br>

  올드 공간에서는 눈에 띄는 변화는 없다.</br>
  => 큰 객체가 테뉴어드 세대애 직접 생성될 때도 있지만, 그 밖에서 영세대 객체가 승격되거나, 올드/풀 수집이 일어나 객체를 재탐색 후 다시 배치하는 등의 수집이 일어날 때만 변한다.</br>
  <h3>병렬 수집기의 한계</h3>

  풀 stw를 유발</br>

  영 수집에서는 큰 문제가 없다.(용량이 크지 않다.)</br>
  하지만, 올드 수집 시 크기가 훨씬거 stw수집이 길어진다.</br>
  또한, 영역 내 살아있는 객체 수만큼 마킹 시간도 늘어난다</br>

  
  <h3>핟당과 역할</h3>

  자바의 가비지 수집 프로세스는 보통 유입된 메모리 할당 요청을 수용하기에 메모리가 부족할 때 작동하여 필요한 만큼 메모리를 공급한다.</br>
  
         
  ![KakaoTalk_20230807_122414515](https://github.com/JSON-loading-and-unloading/Optimizing-Java/assets/106163272/7e91f097-c908-4e3f-84c4-23748811be23)</br>

  에덴은 4초면 다 채워지므로 영GC는 4초마다 발생.</br>

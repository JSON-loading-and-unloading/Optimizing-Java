...
마크 단계 (Mark Phase): GC 루트(root)에서부터 시작하여, 어떤 객체가 여전히 참조되고 있는지 마크합니다. 이는 더 이상 접근할 수 없는 객체들을 식별하기 위한 단계입니다.</br>

스위핑 단계 (Sweep Phase): 할당된 메모리 공간 전체를 검사하면서 마크되지 않은 객체들을 찾아내고, 이들의 메모리를 회수합니다. 이때 회수된 메모리는 다시 재사용 가능한 상태로 만듭니다.</br>

프리 리스트 구축 (Free List Construction): 스위핑 단계에서 회수된 메모리 블록들은 프리 리스트에 추가됩니다. 이는 나중에 객체가 새로 할당되어야 할 때, 이 프리 리스트에서 빠르게 사용 가능한 메모리 블록을 찾아서 할당하는 데 사용됩니다. 이를 통해 메모리 할당의 오버헤드를 줄이고, 힙 내부의 메모리 단편화 문제를 완화할 수 있습니다.</br>

할당 리스트 업데이트 (Allocation List Update): 스위핑 및 프리 리스트 구축 단계 후, 더 이상 필요 없는 객체들은 할당 리스트에서 삭제됩니다. 이 객체들이 차지하던 메모리 공간은 프리 리스트에 되돌려집니다.</br>

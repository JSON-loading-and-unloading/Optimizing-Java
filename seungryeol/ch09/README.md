...
=> 핫스팟은 프로파일링 정보를 보관하지 않고 VM이 꺼지면 일체 폐기한다. 따라서 프로파일은 처음부터 다시 만들어진다.</br>
 ...
 핫스팟은 핫 루프를 온-스택 치환(OSR)이라는 기법을 이용해 컴파일하는 기능도 지원한다.
 OSR은 어떤 메서드가 컴파일할 만큼 자주 호출되지는 않지만, 컴파일하기 적합한 루프가 포함돼 있고 루프 바디 자체가 메서드인 경우 사용한다.


 인터프리터를 진행하다 컴파일 대상으로 낙점된 메서드는 컴파일 스레드에 올려놓고 백그라운드에서 컴파일한다.

 
![KakaoTalk_20230830_150553050_04](https://github.com/JSON-loading-and-unloading/Optimizing-Java/assets/106163272/c7ba979a-a609-43f5-bef7-76963d6af97f)

 최적화된 기계어가 생성되면 해당 Klass의 vtable은 새로 컴파일된 코드를 가리키도록 수정된다.
앞으로 이 메서드를 호출하는 코드는 컴파일드 형식으로 받겠지만, 현재 인터프리티드 형식으로 실행 중인 스레ㅔ드는 끝까지 인터프리티드 모드로 현재 호출을 완료한다.
=> 다음에 다시 호출하면 컴파일드 형식을 들고 간다.


 <h4>JIT 컴파일 로깅</h4>


![KakaoTalk_20230830_150553050_03](https://github.com/JSON-loading-and-unloading/Optimizing-Java/assets/106163272/8c6cd0b5-16ee-4cf9-a934-2c318a04c092)

 스위치를 키면 컴파일 이벤트 록고가 표준출력 스트림에 생성되므로 성능 엔지니어는 이 로그를 보고 어떤 메서드가 컴파일되고 있는지 파악할 수 있다.

 
![KakaoTalk_20230830_150553050_02](https://github.com/JSON-loading-and-unloading/Optimizing-Java/assets/106163272/c425ece3-f22f-493d-a29f-d1b61fa176e6)

 핫스팟 JIT 컴파일러가 어떤 결정을 내렸는지 더 자세한 정보를 얻을 수 있다.
 이전에  아래 플래그를 통해 진단 모드를 해제해야함.
![KakaoTalk_20230830_150553050_01](https://github.com/JSON-loading-and-unloading/Optimizing-Java/assets/106163272/620acdb6-8f3f-4aa9-8ae5-2616c9e6406f)



  <h4>핫스팟 내부의 컴파일러</h4>
핫스팟 JVM에는 c1, c2라는 두 JIT 컴파일러가 있다.</br>
각 클라이언트 컴파일러, 서버 컴파일러라고 부른다.</br>

C1, C2 컴파일러 모두 핵심 측정값, 즉 메서드 호출 횟수에 따라 컴파일이 트리거링된다.</br>
호출 횟수가 특정 한계치에 이르면 그 사실을 VM이 알림 받고 해당 메서드를 컴파일 큐에 넣는다.</br>
=> 컴파일 프로세스는 가장 먼저 메서드의 내부 표현형을 생성한 다음, 인터프리티드 단계에서 수집한 프로파일링 정보를 바탕으로 최적화 로직을 적용한다.</br>
하지만, 같은 코드라도 c1, c2이 생성한 내부 표현형은 전혀 다르다. (c1은 c2보다 컴파일 시간도 더 짧고 단순하게 설치된 까닭에 c2처럼 풀 최적화는 안한다.)</br>


  <h4>핫스팟의 단계별 컴파일</h4>

자바6부터 JVM은 단계별 컴파일모드를 지원</br>
=> 인터프리티드 모드로 실행되다가 단순한 c1 컴파일 형식으로 바뀌고, 다시 이를 c2가 보다 고급 최적화를 수행하는 방식으로 단계를 바꿈</br>

 <h2>코드 캐시</h2>

JIT 컴파일드 코드는 코드 캐시라는 메모리 영역에 저장된다. 이곳에는 인터프리터 부속 등 VM 자체 네이티브 코드가 함께 들어있다.</br>

※VM 시작 시 코드 캐시는 설정된 값으로 최대 크기가 고정되므로 확장이 불가하다.</br>
코드 캐시가 꽉 차면 그때부터 더 이상 JIT파일은 안 되며, 컴파일되지 않은 코드는 인터프리터에서만 실행됨.</br>
코드 캐시는 미할당 영역과 프리 블록 연결 리스트를 담은 힙으로 구현</br>
블록 재활용은 스위퍼라는 프로세스가 담당.</br>

네이티브 메서드가 새로 저장되면 컴파일드 코드를 담기에 크기가 충분한 블록을 프리 리스트에서 찾아본다. 만약 그런 블록이 없으면 여유 공간이 충분한 코드 캐시 사정에 따라 미할당 공간에서 새 블록을 생성</br>

해당 경우, 네이티브 코드는 코드 캐시에서 제거됨</br>
- 역최적화가 될 때
- 다른 컴파일 버전으로 교체됐을 때
- 메서드를 지닌 클래스가 언로딩 될 때


  코드 캐시 최대 크기 조정 플래그</br>
</br>

![KakaoTalk_20230830_150553050](https://github.com/JSON-loading-and-unloading/Optimizing-Java/assets/106163272/3b8ade48-7357-46f9-8fc5-91cb7e5ae7b6)</br>
 ...
 1. PrintComplication 스위치를 켜고 애플리케이션을 실행
